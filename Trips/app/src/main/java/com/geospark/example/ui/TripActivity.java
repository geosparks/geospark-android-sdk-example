...
public class TripActivity extends AppCompatActivity {
    ...
    public void getActiveTrips() {
        showProgressDialog();
        GeoSpark.activeTrips(false, new GeoSparkActiveTripsCallback() {
            @Override
            public void onSuccess(GeoSparkTrip geoSparkTrip) {
                stopProgressDialog();
                if (geoSparkTrip.getActiveTrips().size() != 0) {
                    mAdapter.addAllItem(geoSparkTrip.getActiveTrips());
                } else {
            ...
            public void onFailure(GeoSparkError geoSparkError) {
                stopProgressDialog();
                Util.showToast(TripActivity.this, geoSparkError.getMessage());
            }
        });
    }
